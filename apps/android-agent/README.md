...
当前已验证范围包括私聊文本，以及受 Bridge 白名单、明确 `@`、限流和熔断保护的群聊
文本收发。图片、文件、引用消息和历史消息读取仍不在当前范围。Hook 入口通过微信版本
硬门禁固定为 `8.0.70`；其他微信版本不会
...
推荐从 Monorepo 根目录运行 Windows 工具，它会准备固定并校验过的 JDK 17、Android
SDK 34 和 Gradle 8.9：
...
该参考仓库当前没有声明许可证，因此本项目没有复制或打包其源文件，只依据其公开
说明核对版本化接入点，并使用本项目自己的 Binder、Agent、鉴权与 ACK 实现。
