// FEAGLEwxbot 图片链路测试 v2：假 OneBot client 直连 AstrBot 6199
// 本机猫图 base64 内嵌（base64://），不依赖任何外网
// 验证：AstrBot 收图 → MediaResolver 解码 → 识图模型 → 回复
const WebSocket = require('/app/node_modules/ws');
const fs = require('fs');

const B64 = fs.readFileSync('/tmp/cat-512.b64', 'utf8').trim();
const IMG_FILE = 'base64://' + B64;
...
ws.on('open', () => {
  console.log('TEST-CONNECTED self=' + SELF_ID + ' imgB64Len=' + B64.length);
  ...
  const msg = {
    ...
    user_id: USER_ID,
    message: [
      { type: 'image', data: { file: IMG_FILE } },
      { type: 'text', data: { text: '请用中文描述这张图片里有什么。' } },
    ],
    raw_message: '[CQ:image,file=base64://' + B64.slice(0, 24) + '...] 请用中文描述这张图片里有什么。',
    sender: { user_id: USER_ID, nickname: '识图测试' },
  };
  ws.send(JSON.stringify(msg));
  console.log('TEST-SENT 图片消息 (base64, ' + B64.length + ' chars)');
});
